...
{
    if (ParamSON_CHSonosChannelUsage == PT_SONSonosChannelUsage::Disabled)
        return nullptr;
...
void SonosModule::processInputKo(GroupObject &ko)
{
    ...
    SonosChannelOwnerModule::processInputKo(ko);
}
...
void SonosModule::loop()
{
    ...
    {
        logInfoP("Network connected: %s", openknxNetwork.localIP().toString().c_str());
